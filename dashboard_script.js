...
Rueckmeldung: ${message}</div>
    ...
    function tradingReferencePrice(symbol) {
      ...
      if (cached && Date.now() - cached.at < 60000) return Number(cached.price) || null;
      ...
      const cycleSymbols = latestStatusData?.cycle?.symbols || {};
      const symbolData = cycleSymbols?.[safeSymbol] || cycleSymbols?.[`${safeSymbol}.P`] || null;
      const candlePrice = Number(symbolData?.last_signal_candle?.close ?? symbolData?.last_confirmation_candle?.close);
    ...
    async function loadTradingReferencePrice(symbol, force = false) {
      const safeSymbol = String(symbol || '').toUpperCase();
      ...
      try {
        const response = await fetch(`/api/chart-data?symbol=${encodeURIComponent(safeSymbol)}&resolution=${encodeURIComponent(resolution)}&limit=2`, { cache: 'no-store' });
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || 'Preis nicht geladen');
        const candles = data.candles || [];
        const last = candles[candles.length - 1] || null;
        const price = Number(last?.close);
        if (!Number.isFinite(price) || price <= 0) throw new Error('kein gültiger Close');
        tradingPriceCache[safeSymbol] = { price, at: Date.now() };
    ...
    function renderTradingPreview() {
      ...
      const margin = mode === 'usd' ? usd : (price > 0 ? (asset * price) / leverage : 0);
      const notional = mode === 'usd' ? usd * leverage : (price > 0 ? asset * price : 0);
      const quantity = mode === 'asset' ? asset : (price > 0 ? notional / price : 0);
      const longLiq = price > 0 ? price * (1 - 1 / leverage) : 0;
      const shortLiq = price > 0 ? price * (1 + 1 / leverage) : 0;
    ...
    function clampTradingMarginToAvailable(value) {
      const margin = Math.max(0, Number(value) || 0);
      const available = tradingAvailableBalance();
      return available > 0 ? Math.min(margin, available) : margin;
    }
    ...
    function applyTradingMarginToActiveMode(margin, forcePriceLoad = true) {
      ...
      const safeMargin = clampTradingMarginToAvailable(margin);
      if (mode === 'usd') {
    ...
    function setTradingRiskPercent(value) {
      ...
      const display = percent === 0 ? '0' : Number(percent.toFixed(2)).toString();
    ...
    function syncTradingRiskFromAmount() {
      ...
      const usd = Number(document.getElementById('sizeUsd')?.value || 0);
      const asset = Number(document.getElementById('sizeAsset')?.value || 0);
      let margin = mode === 'asset' && price > 0 ? (asset * price) / leverage : usd;
      if (available > 0 && margin > available) {
        margin = available;
        applyTradingMarginToActiveMode(available, false);
      }
    ...
    function loadTradeSizeForSelectedAsset() {
      ...
      const size = localTradeSizes[symbol] || {
        mode: latestConfig.trade_size_mode || 'asset',
